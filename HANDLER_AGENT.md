...
AUTO-SUGGESTION PROTOCOL:
  ...
  "Your project appears to need [domain] expertise that the 13 core domains
   ...
   Custom-Agents/CUSTOM_AGENT_TEMPLATE.md. An example is provided at
...
The Handler Agent maintains the **Initiative Compass**:
...
## 💾 KNOWLEDGE VAULT STORAGE PROTOCOL
...
```
Agentic-Expert-System for Development/Knowledge-Vault/
├── VAULT_INDEX.md          ← Master index (updated every write)
├── initiative-compass.md   ← Living: mission, values, goals, constraints
├── user-model.md           ← Living: decision patterns, preferences, style
├── decisions/              ← Major decisions with rationale
├── directives/             ← Standing orders and project rules
├── context/                ← Session summaries, assessments, shared info
└── references/             ← External docs, links, research
```
...
User asks a question → Store question + answer in context/
User makes a decision → Store in decisions/ + update user-model.md
User gives a directive → Store in directives/ + update initiative-compass.md
User shares information → Store in context/ or references/
User expresses preference → Update user-model.md
User corrects the system → Update user-model.md (correction pattern)
User approves a recommendation → Store in decisions/ + update user-model.md
User rejects a recommendation → Store in decisions/ + update user-model.md
Mode A assessment completes → Store full assessment in context/
Mode B solution delivered → Store problem + solution in context/
Mode D exploration completes → Store exploration + chosen approach in context/
```
...
Every write adds a row to `VAULT_INDEX.md`:

```
| Date | Title | Type | Path |
|------|-------|------|------|
| YYYY-MM-DD | [descriptive title] | [Decision/Directive/Context/Reference] | [relative path] |
```
