...
description: "Break large epics and features into isolated, numbered build sessions to prevent context window overflow and AI hallucinations. Each session is scoped, focused, and handed to the executor agent one at a time."
user-invocable: true
...
3. **Generate Numbered Session Files:**
   ...
   - Each session file must contain:
     ...
     - **Knowledge Logging:** Explicit instruction to the agent to summarize all architectural decisions, bug fixes, and new dependencies from this session and append them to `Knowledge-Vault/session_logs.md` before concluding the session.
...
5. **Consolidate Memory (End of Build):**
   - When the final session of the build is complete, automatically trigger the Librarian memory consolidation workflow to synthesize the `Knowledge-Vault/session_logs.md` into the permanent system domains.
