...
All domain agent files are at: `${CLAUDE_SKILL_DIR}/domains/`
...
Supporting files:
- Knowledge Vault: `${CLAUDE_SKILL_DIR}/knowledge-vault/`
- Custom Agents: `${CLAUDE_SKILL_DIR}/custom-agents/`
...
For each PRIMARY domain, read its full file from `${CLAUDE_SKILL_DIR}/domains/`.
...
Store/update compass at `${CLAUDE_SKILL_DIR}/knowledge-vault/initiative-compass.md`
Store/update user model at `${CLAUDE_SKILL_DIR}/knowledge-vault/user-model.md`
...
Check `${CLAUDE_SKILL_DIR}/custom-agents/AGENT_REGISTRY.md` for active custom agents. Custom agents:
...
Template for new custom agents: `${CLAUDE_SKILL_DIR}/custom-agents/CUSTOM_AGENT_TEMPLATE.md`

---

## WORKFLOWS

Three built-in workflows extend the core parliamentary system. Their full definitions are in `${CLAUDE_SKILL_DIR}/workflows/`.

### /Session-Wise Build Planning
For building large epics or features. The Handler breaks the build into isolated, numbered sessions (`session_01_db.md`, `session_02_api.md`), each scoped to fit within a context window. The executor agent receives exactly ONE session at a time and stays blind to future sessions to maintain focus. Each session file contains: Goal, Input Context, Implementation Steps, Success Criteria, Strict Boundaries, and Knowledge Logging instructions.

### /Librarian - Memory Consolidation
A lightweight agent that preserves project memory. After significant decisions or at session end:
1. Reads `Knowledge-Vault/session_logs.md` for raw session notes
2. Categorizes learnings (ADRs, security constraints, strategy, resolved bugs)
3. Surgically inserts new knowledge into the appropriate permanent files
4. Clears the temporary session log
5. Reports what was stored and where

**CRITICAL:** The Librarian does NOT write code or design features. It only consolidates memory. It NEVER deletes or overwrites existing constraints unless explicitly told a constraint was deprecated.

### Memory Consolidation Integration
After delivering a final recommendation (Step 5), ALWAYS:
1. Summarize the core architectural decisions and adopted strategies
2. Append the summary to `Knowledge-Vault/session_logs.md`
3. If the session is ending or a major decision was finalized, trigger the Librarian workflow
