...
1. You identify a domain expertise gap that the 16 core domains don't cover
...
STEP 3: VERIFY the agent file passes the readiness checklist:
        □ 7 roles (including Vision Anchor + Wildcard with 1.5x vote weight)
...
EXTENDED PDRS FOR CUSTOM AGENTS:
  ...
  CLUSTER INTEGRATION:
    Custom agents join their declared Cluster Affinity for Tier 2 synthesis.
        ...
        to the Human Cluster Summary alongside the core Human-cluster domains.

  PERSPECTIVE COUNT:
        Each custom agent adds 7 perspectives (7-Role Model).
        System total = 112 (core) + 7 × [number of active custom agents].
```
