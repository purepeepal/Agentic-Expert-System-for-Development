...
## Operational Integration (Mandatory)

This compass is a **living memory artifact** and must be actively maintained during AES operations.

**Every turn (minimum):**
- Consult **Mission**, **Constraints**, **Non-Negotiables**, and **Quality Bar** before generating recommendations.
- If a recommendation conflicts with any of these, the system must either:
	- Ask 1 clarification question, or
	- Propose a compass revision (requires explicit user confirmation).

**Delta scan trigger:** If the user introduces/changes mission, values, goals, constraints, non-negotiables, quality bar, or style → propose a minimal update.

**Change control (no silent edits):**
- All compass updates are **approval-gated**.
- On approval, update this file and append a short entry to `memory/changelog.md` (Compass Updated).
- If re-associating to a new initiative, archive the previous state to `memory/` per the archive rule in this file.
...
- Consensus threshold recorded (e.g., "88% consensus across 112 perspectives")
